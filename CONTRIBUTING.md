# Contributing to MongrelDB .NET

Thanks for taking the time to help the MongrelDB .NET client. This document
...
The MongrelDB .NET client uses a standard **fork → branch → pull request**
workflow on GitHub.

1. **Fork** [`visorcraft/MongrelDB-DotNet`](https://github.com/visorcraft/MongrelDB-DotNet)
   to your GitHub account.
2. **Clone** your fork and add the upstream remote:

   ```sh
   git clone git@github.com:<you>/MongrelDB-DotNet.git
   cd MongrelDB-DotNet
   git remote add upstream https://github.com/visorcraft/MongrelDB-DotNet.git
   ```
...
5. **Open a pull request** against `master` on `visorcraft/MongrelDB-DotNet`.
...
```sh
MONGRELDB_URL=http://127.0.0.1:8453 dotnet test --configuration Release --no-build --logger "console;verbosity=normal"
```
...
- **Version.** .NET 8.0 or newer. Don't drop the minimum casually.
...
- **Async.** All I/O methods are async (`*Async` suffix, return `Task` /
  `Task<T>`). No synchronous blocking I/O.
- **Naming.** Idiomatic C#: `PascalCase` types and public members,
...
- The MongrelDB .NET client version (from `MongrelDB.csproj` / NuGet).
...
The MongrelDB .NET client is dual-licensed under MIT OR Apache-2.0. By
