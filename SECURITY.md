...
This document describes the security properties of the MongrelDB .NET client
and how to report vulnerabilities.

## Overview

The MongrelDB .NET client is a C# library that talks to `mongreldb-server`
over HTTP using the standard library `System.Net.Http.HttpClient` and
`System.Text.Json`. The client itself holds no encryption keys and stores
...
- The transport is pluggable: bring your own `HttpClient`.
...
- Server and network errors are mapped to the typed exception hierarchy
  ...
  not leaked as generic exceptions.

## Dependency security

The MongrelDB .NET client has no runtime dependencies beyond the .NET base
class library. Report dependency vulnerabilities through GitHub's
...
- the MongrelDB .NET client version, .NET version, and OS,
